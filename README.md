...
├── enable-autostart.sh             # Enable auto-start on existing installations
...
Built specifically for Podman instead of Docker. Uses direct bind mounts, systemd service for auto-start on boot, systemd timers for update checks, and SELinux-compatible volume labels (`:Z`).
...
sudo systemctl status n8n-stack.service

# Start/stop/restart the stack
sudo systemctl start n8n-stack.service
sudo systemctl stop n8n-stack.service
...
# View service logs
sudo journalctl -u n8n-stack.service -f
```

For existing installations that want to add auto-start:
